      ...
      <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 px-5 sm:px-0 mt-10 gap-6'>
            {data.map((movie) => (
                <div key={movie.Movie?.id} className='relative h-60'>
                    <Image src={movie.Movie?.imageString as string} alt='Movie' width={500} height={400} className='rounded-sm absolute w-full h-full object-cover' />
                    <div className='h-60 relative z-10 w-full transform transition duration-500 hover:scale-125 opacity-0 hover:opacity-100'>
                    <div className='bg-gradient-to-b from-transparent via-black/50 to-black z-10 w-full h-full rounded-lg flex items-center justify-center border'>
                            <Image src={movie.Movie?.imageString as string} alt='Movie' width={800} height={800} className='absolute w-full h-full -z-10 rounded-lg object-cover' />
                            <MovieCard
