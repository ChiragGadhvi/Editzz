...
import { InfoIcon, Key, PlayCircle } from "lucide-react";
...
export default function MovieButtons({age,duration,id,overview,releaseDate,title,youtubeUrl} : iAppProps) {
    ...
    return (
        ...
        <PlayVideoModal state={open} changeState={setOpen} age={age} duration={duration} overview={overview} release={releaseDate} title={title} youtubeUrl={youtubeUrl} key={id} />
        </>
    )
}